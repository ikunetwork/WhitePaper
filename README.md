...
In addition, capital is almsot exclusively allocated to new molecular entities NME - molecules never conceived nor had human exposure, as NME's are a more enforceable record of data \(because they never existed before\).[5](./#f5) Unfortunately, NME's are logistically difficult to achieve, requiring:
...
Typically, by the time a NME is ready to enter clinical trial, there is &lt; 10 years left of patent life, often times only 5-6 years.[18](./#f18) Jurisdictions including U.S., EU and Asia provide market/data exclusivity periods \(independent of patents\) for clinical trial data, regardless if an NME, for up to ~10 years.[19](./#f19) Generic medicines are built on the same active ingredients \(as the patented drugs\) once their patents or marketing exclusivities expire.[20](./#f20) The same dosage, effectiveness and - safety - profile as the original drug as well.[21](./#f21) By definition, once the generic forms can be sold, the ingredient is on the market for many years and as such, generics themeselves are considered safe for patients.[22](./#f22) Because their patents expired, there is significantly less risk of infringement and more freedom to operate.

The IN provides for a two token incentive model to secure the network. `IKU` runs the IN and `RST[x]` are digital rights to R&D data generated by `RST[x]` holders. `RSTx` holders operate as a  `RST[x]DC` and may agree to pursue a patent together as a DC, with sole ownership of rights by the DC. While `RSTx` holders may pursue patent protection for inventions developed based on IN funded research, patents may not be used in the IN. The `RST[x]` Holder has pro-rata rights in the R&D data if they hold the respective `RST[x]`. The IN is designed to provide global liquidity for R&D regardless of patent policy.
